...
class Seq2SeqForAddress(object):
    ...
    def model_fn(self, features, labels, params, mode):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
            out_seq = labels["output"]
            out_seq_length = labels["output_length"]
        else:
            out_seq = None
            out_seq_length = None
        table = lookup.index_table_from_file(params["vocabs_features_file"], default_value=0)
        ...
        with tf.variable_scope("embedding"):
            embedding = tf.Variable(tf.random.uniform([vocab_size, num_units], -1.0, 1.0), dtype=tf.float32,
                                    name='word_embedding')
            # embedding = tf.compat.v1.get_variable(name='embed', shape=[vocab_size, num_units])
        ...
        bi_encoder_output, bi_encoder_state = tf.nn.bidirectional_dynamic_rnn(cell_fw=encoder_cell_fw,
                                                                              ...
                                                                              dtype=embed_input.dtype, time_major=False)
        ...
        final_outputs, final_state, final_sequence_lengths = tf.contrib.seq2seq.dynamic_decode(decoder=basic_decoder,
        ...
        if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
            ...
            train_op = tf.compat.v1.train.AdamOptimizer(learning_rate=0.001).apply_gradients(
            ...
            index_to_string = lookup.index_to_string_table_from_file(params["vocabs_features_file"])
    ...
    def _get_final_predictions(self, outputs):
        # 怎么转换模型输出为最终预测结果？？？
        pass
